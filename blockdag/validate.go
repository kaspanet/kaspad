...
func CheckTransactionSanity(tx *util.Tx, subnetworkID *subnetworkid.SubnetworkID, isFeeTransaction bool) error {
	...
	if (msgTx.SubnetworkID == subnetworkid.SubnetworkIDNative ||
		msgTx.SubnetworkID == subnetworkid.SubnetworkIDRegistry) &&
		msgTx.Gas > 0 {
	...
	if msgTx.SubnetworkID == subnetworkid.SubnetworkIDRegistry &&
		len(msgTx.Payload) != 8 {
