...
func TestCheckTransactionSanity(t *testing.T) {
	tests := []struct {
		...
		{"no inputs", 0, 1, 1, *subnetworkid.SubnetworkIDNative, nil, nil, ruleErrorFromString(ErrNoTxInputs, "")},
		{"no outputs", 1, 0, 1, *subnetworkid.SubnetworkIDNative, nil, nil, nil},
		{"too massive", 1, 1000000, 1, *subnetworkid.SubnetworkIDNative, nil, nil, ruleErrorFromString(ErrTxMassTooHigh, "")},
		{"too much sompi in one output", 1, 1, util.MaxSompi + 1,
			*subnetworkid.SubnetworkIDNative,
			nil,
			nil,
			ruleErrorFromString(ErrBadTxOutValue, "")},
		...
		{"duplicate inputs", 2, 1, 1,
			*subnetworkid.SubnetworkIDNative,
			...
			ruleErrorFromString(ErrDuplicateTxInputs, "")},
		{"1 input coinbase",
			1,
			1,
			1,
		...
		{"non-zero payload in Kaspa", 1, 1, 0,
			*subnetworkid.SubnetworkIDNative,
			...
			nil,
			ruleErrorFromString(ErrInvalidPayload, "")},
		...
		{"invalid payload hash", 1, 1, 0,
			...
			ruleErrorFromString(ErrInvalidPayloadHash, "")},
