...
func (dag *BlockDAG) ghostdag(newNode *blockNode) (selectedParentAnticone []*blockNode, err error) {
	...
	for _, blueCandidate := range selectedParentAnticone {
		candidateBluesAnticoneSizes := make(map[*blockNode]uint8)
		var candidateAnticoneSize uint8
		possiblyBlue := true
		...
		if possiblyBlue {
			...
			if uint8(len(newNode.blues)) == dag.dagParams.K+1 {
				break
			}
		}
	}
...
func (dag *BlockDAG) blueAnticoneSize(block, context *blockNode) (uint8, error) {
