	...
	for _, blueCandidate := range selectedParentAnticone {
		...
		for chainBlock := newNode; possiblyBlue; chainBlock = chainBlock.selectedParent {
			...
			if chainBlock != newNode {
				if isAncestorOfBlueCandidate, err := dag.isInFuture(chainBlock, blueCandidate); err != nil {
					return nil, err
			...
			for _, block := range chainBlock.blues {
				...
				if isAncestorOfBlueCandidate, err := dag.isInFuture(block, blueCandidate); err != nil {
					return nil, err
				} else if isAncestorOfBlueCandidate {
					continue
				}
...
func (dag *BlockDAG) selectedParentAnticone(node *blockNode) ([]*blockNode, error) {
	...
	for len(queue) > 0 {
		...
		for parent := range current.parents {
			...
			isAncestorOfSelectedParent, err := dag.isInFuture(parent, node.selectedParent)
			if err != nil {
				return nil, err
			}
