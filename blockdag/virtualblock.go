...
type virtualBlock struct {
	...
	utxoSet  *FullUTXOSet
	blockNode
	// selectedPathChain is a block set that includes all the blocks that belong to the chain of selected parents from the virtual block.
	selectedPathChain blockSet
}
...
func newVirtualBlock(tips blockSet, phantomK uint32) *virtualBlock {
	...
	virtual.selectedPathChain = newSet()
...
func (v *virtualBlock) clone() *virtualBlock {
	return &virtualBlock{
		...
		selectedPathChain: v.selectedPathChain,
	}
}
...
func (v *virtualBlock) updateSelectedPathSet(oldSelectedParent *blockNode) {
	...
	for node := v.blockNode.selectedParent; intersectionNode == nil && node != nil; node = node.selectedParent {
		if v.selectedPathChain.contains(node) {
			intersectionNode = node
		} else {
			v.selectedPathChain.add(node)
		}
	}
	...
	if intersectionNode != nil {
		for node := oldSelectedParent; !node.hash.IsEqual(&intersectionNode.hash); node = node.selectedParent {
			v.selectedPathChain.remove(node)
		}
	}
}
