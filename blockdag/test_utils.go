...
// VirtualForTest is an exported version for virtualBlock, so that i can be returned by exported test_util methods
type VirtualForTest struct{ *virtualBlock }
...
func SetVirtualForTest(dag *BlockDAG, virtual *VirtualForTest) *VirtualForTest {
	...
	dag.virtual = virtual.virtualBlock
