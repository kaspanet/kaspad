...
func (dag *BlockDAG) antiPastBetween(lowHash, highHash *daghash.Hash, maxEntries uint64) ([]*blockNode, error) {
	...
	if !ok {
		return nil, errors.Wrapf(InvalidParameterError, "Couldn't find low hash %s", lowHash)
	}
	highNode, ok := dag.index.LookupNode(highHash)
	if !ok {
		return nil, errors.Wrapf(InvalidParameterError, "Couldn't find high hash %s", highHash)
	}
	if lowNode.blueScore >= highNode.blueScore {
		return nil, errors.Wrapf(InvalidParameterError, "Low hash blueScore >= high hash blueScore (%d >= %d)",
