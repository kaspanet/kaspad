...
func txLoop(client *txgenClient, cfg *commandConfig) error {
...
func createRandomTxFromFunds(walletUTXOSet utxoSet, cfg *commandConfig, gasLimitMap map[subnetworkid.SubnetworkID]uint64, funds uint64) (tx *wire.MsgTx, isSecondaryAddress bool, err error) {
...
func enqueueTransactions(client *txgenClient, blockAdded *blockAddedMsg, walletUTXOSet utxoSet, walletTxs map[daghash.TxID]*walletTransaction,
	txChan chan *wire.MsgTx, cfg *commandConfig, gasLimitMap map[subnetworkid.SubnetworkID]uint64) error {
