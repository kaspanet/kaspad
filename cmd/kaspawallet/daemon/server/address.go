...
func (s *server) changeAddress() (util.Address, error) {
	...
	return libkaspawallet.Address(s.cfg.NetParams(), s.keysFile.ExtendedPublicKeys, s.keysFile.MinimumSignatures, path, s.keysFile.ECDSA)
}
...
func (s *server) GetReceiveAddress(_ context.Context, request *pb.GetReceiveAddressRequest) (*pb.GetReceiveAddressResponse, error) {
	...
	address, err := libkaspawallet.Address(s.cfg.NetParams(), s.keysFile.ExtendedPublicKeys, s.keysFile.MinimumSignatures, path, s.keysFile.ECDSA)
	if err != nil {
		return nil, err
	}
