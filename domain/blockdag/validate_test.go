...
func TestPastMedianTime(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		tip = newTestNode(dag, blocknode.SetFromSlice(tip),
			blockVersion,
			0,
			blockTime)
	}
	...
	node := newTestNode(dag, blocknode.SetFromSlice(tip),
		blockVersion,
		dag.powMaxBits,
...
func TestValidateParents(t *testing.T) {
	...
	err = dag.validateParents(fakeBlockHeader, blocknode.SetFromSlice(aNode, bNode))
	if err == nil {
		t.Errorf("validateParents: `a` is a parent of `b`, so an error is expected")
	}

	// Check indirect parents relation
	err = dag.validateParents(fakeBlockHeader, blocknode.SetFromSlice(dag.genesis, bNode))
	if err == nil {
		t.Errorf("validateParents: `genesis` and `b` are indirectly related, so an error is expected")
	}

	// Check parents with no relation
	err = dag.validateParents(fakeBlockHeader, blocknode.SetFromSlice(bNode, cNode))
	if err != nil {
