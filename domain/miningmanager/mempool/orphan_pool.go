...
func (op *orphansPool) addOrphan(transaction *externalapi.DomainTransaction, isHighPriority bool) error {
	virtualDAAScore, err := op.mempool.consensusWrapper.Consensus().GetVirtualDAAScore()
	if err != nil {
		return err
	}
...
func (op *orphansPool) unorphanTransaction(transaction *model.OrphanTransaction) error {
	...
	err = op.mempool.consensusWrapper.Consensus().ValidateTransactionAndPopulateWithConsensusData(transaction.Transaction())
	if err != nil {
