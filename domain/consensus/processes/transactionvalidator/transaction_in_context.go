...
func (v *transactionValidator) calcTxSequenceLockFromReferencedUTXOEntries(
	...
	for _, input := range tx.Inputs {
		...
		case sequenceNum&constants.SequenceLockTimeIsSeconds == constants.SequenceLockTimeIsSeconds:
			...
			baseGHOSTDAGData, err := v.ghostdagDataStore.Get(v.databaseContext, nil, povBlockHash)
			if err != nil {
				return nil, err
			}
			...
			for {
				...
				selectedParentGHOSTDAGData, err := v.ghostdagDataStore.Get(v.databaseContext, nil, baseGHOSTDAGData.SelectedParent())
				if err != nil {
					return nil, err
				}
