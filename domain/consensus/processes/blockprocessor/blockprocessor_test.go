...
func setupTestContext(dagParams *dagconfig.Params, dbManager model.DBManager) (*testContext, error) {
	...
	consensusStateStore := consensusstatestore.New(200)
...
func generateScriptPublicKey(publicKey *secp256k1.SchnorrPublicKey) (*externalapi.ScriptPublicKey, error) {
...
func NewCoinbaseData(scriptPublicKey *externalapi.ScriptPublicKey) (*externalapi.DomainCoinbaseData, error) {
...
func TestBlockProcessor(t *testing.T) {
	createChain := func(t *testing.T, numOfBlocks int) (model.BlockProcessor, []*externalapi.DomainBlock, func()) {
		...
		if err != nil {
			t.Fatalf("error in SetupCoinbaseData: %+v", err)
		}
