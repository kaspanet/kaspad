...
func (sm *syncManager) createBlockLocator(lowHash, highHash *externalapi.DomainHash, limit uint32) (externalapi.BlockLocator, error) {
	...
	for {
		...
		currentBlockGHOSTDAGData, err := sm.ghostdagDataStore.Get(sm.databaseContext, nil, currentHash)
		if err != nil {
			return nil, err
		}
...
func (sm *syncManager) createHeadersSelectedChainBlockLocator(lowHash,
	...
	lowHashIndex, err := sm.headersSelectedChainStore.GetIndexByHash(sm.databaseContext, nil, lowHash)
	if err != nil {
	...
	highHashIndex, err := sm.headersSelectedChainStore.GetIndexByHash(sm.databaseContext, nil, highHash)
	if err != nil {
		if database.IsNotFoundError(err) {
			return nil, errors.Wrapf(model.ErrBlockNotInSelectedParentChain,
				"LowHash %s is not in selected parent chain", lowHash)
	...
	for currentIndex > lowHashIndex {
		blockHash, err := sm.headersSelectedChainStore.GetHashByIndex(sm.databaseContext, nil, currentIndex)
		if err != nil {
			return nil, err
		}
