...
type blockHeaderStore struct {
	staging  map[externalapi.DomainHash]externalapi.ImmutableBlockHeader
...
func New(dbContext model.DBReader, cacheSize int) (model.BlockHeaderStore, error) {
	blockHeaderStore := &blockHeaderStore{
		staging:  make(map[externalapi.DomainHash]externalapi.ImmutableBlockHeader),
...
func (bhs *blockHeaderStore) Stage(blockHash *externalapi.DomainHash, blockHeader externalapi.ImmutableBlockHeader) {
...
func (bhs *blockHeaderStore) Discard() {
	bhs.staging = make(map[externalapi.DomainHash]externalapi.ImmutableBlockHeader)
...
func (bhs *blockHeaderStore) BlockHeader(dbContext model.DBReader, blockHash *externalapi.DomainHash) (externalapi.ImmutableBlockHeader, error) {
	if header, ok := bhs.staging[*blockHash]; ok {
		return header, nil
	}

	if header, ok := bhs.cache.Get(blockHash); ok {
		return header.(externalapi.ImmutableBlockHeader), nil
	}
...
func (bhs *blockHeaderStore) BlockHeaders(dbContext model.DBReader, blockHashes []*externalapi.DomainHash) ([]externalapi.ImmutableBlockHeader, error) {
	headers := make([]externalapi.ImmutableBlockHeader, len(blockHashes))
...
func (bhs *blockHeaderStore) serializeHeader(header externalapi.ImmutableBlockHeader) ([]byte, error) {
	dbBlockHeader := serialization.DomainBlockHeaderToDbBlockHeader(header)
	return proto.Marshal(dbBlockHeader)
}

func (bhs *blockHeaderStore) deserializeHeader(headerBytes []byte) (externalapi.ImmutableBlockHeader, error) {
