...
func (s *consensus) EstimateNetworkHashesPerSecond(blockHash *externalapi.DomainHash, windowSize int) (uint64, error) {
	s.lock.Lock()
	defer s.lock.Unlock()

	return s.difficultyManager.EstimateNetworkHashesPerSecond(blockHash, windowSize)
}
