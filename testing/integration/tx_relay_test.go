...
func TestTxRelay(t *testing.T) {
	...
	spawn("TestTxRelay-WaitForTransactionPropagation", func() {
		...
		for range ticker.C {
			...
			_, err := payee.rpcClient.GetMempoolEntry(txID, false, true)
			if err != nil {
			...
			mempoolEntriesByAddresses, err := payee.rpcClient.GetMempoolEntriesByAddresses(mempoolAddressQuery, false, true)
			if err != nil {
				t.Fatalf("Error getting mempool entry: %+v", err)
			}
