...
func handleSearchRawTransactions(s *Server, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*kaspajson.SearchRawTransactionsCmd)
	vinExtra := false
	if c.VinExtra != nil {
		vinExtra = *c.VinExtra
	}
	...
	if len(addressTxns) == 0 {
		return []kaspajson.SearchRawTransactionsResult{}, nil
	}
	...
	srtList := make([]kaspajson.SearchRawTransactionsResult, len(addressTxns))
	for i := range addressTxns {
...
// passed transaction.
func createVinListPrevOut(s *Server, mtx *wire.MsgTx, chainParams *dagconfig.Params, vinExtra bool, filterAddrMap map[string]struct{}) ([]kaspajson.VinPrevOut, error) {
	// Use a dynamically sized list to accommodate the address filter.
	vinList := make([]kaspajson.VinPrevOut, 0, len(mtx.TxIn))
	...
	for _, txIn := range mtx.TxIn {
		...
		if vinExtra {
			...
			vinListEntry.PrevOut = &kaspajson.PrevOut{
				Address: encodedAddr,
