...
func (c *RPCClient) GetBlock(hash string, subnetworkID string, includeTransactionVerboseData bool) (
	*appmessage.GetBlockResponseMessage, error) {

	err := c.rpcRouter.outgoingRoute().Enqueue(
		appmessage.NewGetBlockRequestMessage(hash, subnetworkID, includeTransactionVerboseData))
	if err != nil {
		return nil, err
	}
