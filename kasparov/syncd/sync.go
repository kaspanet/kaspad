...
func syncBlocks(client *jsonrpc.Client) error {
	...
	var verboseBlocks []kaspajson.GetBlockVerboseResult
	for {
...
func addBlocks(client *jsonrpc.Client, rawBlocks []string, verboseBlocks []kaspajson.GetBlockVerboseResult) error {
...
func addBlock(client *jsonrpc.Client, rawBlock string, verboseBlock kaspajson.GetBlockVerboseResult) error {
	db, err := database.DB()
	if err != nil {
		return err
	}
...
func insertBlock(dbTx *gorm.DB, verboseBlock kaspajson.GetBlockVerboseResult) (*dbmodels.Block, error) {
...
func insertBlockParents(dbTx *gorm.DB, verboseBlock kaspajson.GetBlockVerboseResult, dbBlock *dbmodels.Block) error {
...
func insertSubnetwork(dbTx *gorm.DB, transaction *kaspajson.TxRawResult, client *jsonrpc.Client) (*dbmodels.Subnetwork, error) {
...
func insertTransaction(dbTx *gorm.DB, transaction *kaspajson.TxRawResult, dbSubnetwork *dbmodels.Subnetwork) (*dbmodels.Transaction, error) {
...
func calcTxMass(dbTx *gorm.DB, transaction *kaspajson.TxRawResult) (uint64, error) {
...
func convertTxRawResultToMsgTx(tx *kaspajson.TxRawResult) (*wire.MsgTx, error) {
...
func insertTransactionInputs(dbTx *gorm.DB, transaction *kaspajson.TxRawResult, dbTransaction *dbmodels.Transaction) error {
...
func isTransactionCoinbase(transaction *kaspajson.TxRawResult) (bool, error) {
...
func insertTransactionInput(dbTx *gorm.DB, dbTransaction *dbmodels.Transaction, input *kaspajson.Vin) error {
...
func insertTransactionOutputs(dbTx *gorm.DB, transaction *kaspajson.TxRawResult, dbTransaction *dbmodels.Transaction) error {
...
func insertTransactionOutput(dbTx *gorm.DB, dbTransaction *dbmodels.Transaction,
	output *kaspajson.Vout, scriptPubKey []byte, dbAddress *dbmodels.Address) error {
...
// Note that if this function may take a nil dbTx, in which case it would start
// a database transaction by itself and commit it before returning.
func updateSelectedParentChain(removedChainHashes []string, addedChainBlocks []kaspajson.ChainBlock) error {
	db, err := database.DB()
	if err != nil {
		return err
	}
...
func updateRemovedChainHashes(dbTx *gorm.DB, removedHash string) error {
	...
	dbResult = dbTx.
		...
		Where(&dbmodels.Block{AcceptingBlockID: kaspajson.Uint64(dbBlock.ID)}).
...
// This function will return an error if any of the above are in an unexpected state
func updateAddedChainBlocks(dbTx *gorm.DB, addedBlock *kaspajson.ChainBlock) error {
	...
	for _, acceptedBlock := range addedBlock.AcceptedBlocks {
		...
		dbAccepedBlock.AcceptingBlockID = kaspajson.Uint64(dbAddedBlock.ID)
...
type rawAndVerboseBlock struct {
	rawBlock     string
	verboseBlock *kaspajson.GetBlockVerboseResult
}
...
func convertChainChangedMsg(chainChanged *jsonrpc.ChainChangedMsg) (
	removedHashes []string, addedBlocks []kaspajson.ChainBlock) {
	...
	addedBlocks = make([]kaspajson.ChainBlock, len(chainChanged.AddedChainBlocks))
	for i, addedBlock := range chainChanged.AddedChainBlocks {
		acceptedBlocks := make([]kaspajson.AcceptedBlock, len(addedBlock.AcceptedBlocks))
		for j, acceptedBlock := range addedBlock.AcceptedBlocks {
			...
			acceptedBlocks[j] = kaspajson.AcceptedBlock{
				Hash:          acceptedBlock.Hash.String(),
				AcceptedTxIDs: acceptedTxIDs,
			}
		}
		addedBlocks[i] = kaspajson.ChainBlock{
