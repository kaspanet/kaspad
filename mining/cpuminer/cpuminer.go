...
func (m *CPUMiner) submitBlock(block *util.Block) bool {
	...
	if !daghash.AreEqual(msgBlock.Header.PrevBlocks, m.g.GetVirtualBlock().TipHashes()) {
	...
	for extraNonce := uint64(0); extraNonce < maxExtraNonce; extraNonce++ {
		...
		for i := uint32(0); i <= maxNonce; i++ {
			...
			case <-ticker.C:
				...
				virtualBlock := m.g.GetVirtualBlock()
...
out:
	for {
		...
		m.submitBlockLock.Lock()
		curHeight := m.g.GetVirtualBlock().SelectedTipHeight()
