		...
		// Positive P2PKH tests.
		{
			...
			f: func() (btcutil.Address, error) {
				pkHash := []byte{
					0xe3, 0x4c, 0xce, 0x70, 0xc8, 0x63, 0x73, 0x27, 0x3e, 0xfc,
					0xc5, 0x4c, 0xe7, 0xd2, 0xa4, 0x91, 0xbb, 0x4a, 0x0e, 0x84}
				return btcutil.NewAddressPubKeyHash(pkHash, &dagconfig.MainNetParams)
			},
		...
		},
		{
			...
			f: func() (btcutil.Address, error) {
				pkHash := []byte{
					0x78, 0xb3, 0x16, 0xa0, 0x86, 0x47, 0xd5, 0xb7, 0x72, 0x83,
					0xe5, 0x12, 0xd3, 0x60, 0x3f, 0x1f, 0x1c, 0x8d, 0xe6, 0x8f}
				return btcutil.NewAddressPubKeyHash(pkHash, &dagconfig.TestNet3Params)
			},
		...
		// Positive P2SH tests.
		{
			...
			f: func() (btcutil.Address, error) {
				script := []byte{
					...
					0xae}
				return btcutil.NewAddressScriptHash(script, &dagconfig.MainNetParams)
			},
			net: &dagconfig.MainNetParams,
		},
		{
			...
			f: func() (btcutil.Address, error) {
				hash := []byte{
					0xe8, 0xc3, 0x00, 0xc8, 0x79, 0x86, 0xef, 0xa8, 0x4c, 0x37,
					0xc0, 0x51, 0x99, 0x29, 0x01, 0x9e, 0xf8, 0x6e, 0xb5, 0xb4}
				return btcutil.NewAddressScriptHashFromHash(hash, &dagconfig.MainNetParams)
			},
			net: &dagconfig.MainNetParams,
		},
		{
			...
			f: func() (btcutil.Address, error) {
				hash := []byte{
					0xc5, 0x79, 0x34, 0x2c, 0x2c, 0x4c, 0x92, 0x20, 0x20, 0x5e,
					0x2c, 0xdc, 0x28, 0x56, 0x17, 0x04, 0x0c, 0x92, 0x4a, 0x0a}
				return btcutil.NewAddressScriptHashFromHash(hash, &dagconfig.TestNet3Params)
			},
			net: &dagconfig.TestNet3Params,
		},
...
func TestDecodeAddressErrorConditions(t *testing.T) {
	...
	for _, test := range tests {
		_, err := btcutil.DecodeAddress(test.address, &dagconfig.Params{Prefix: test.prefix})
		if err == nil {
