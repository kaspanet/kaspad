...
import (
	...
	"github.com/kaspanet/kaspad/database2/drivers/ffldb/internal/treap"
...
func makeDbErr(c ErrorCode, desc string, err error) Error {
...
func convertErr(desc string, ldbErr error) Error {
	...
	return Error{ErrorCode: code, Description: desc, Err: ldbErr}
}
...
func (c *cursor) Delete() error {
	...
	if !c.bucket.tx.writable {
		...
		return makeDbErr(ErrTxNotWritable, str, nil)
	}

	// Error if the cursor is exhausted.
	if c.currentIter == nil {
		str := "cursor is exhausted"
		return makeDbErr(ErrIncompatibleValue, str, nil)
	}
...
func (b *bucket) CreateBucket(key []byte) (*bucket, error) {
	...
	if !b.tx.writable {
		...
		return nil, makeDbErr(ErrTxNotWritable, str, nil)
	}

	// Ensure a key was provided.
	if len(key) == 0 {
		str := "create bucket requires a key"
		return nil, makeDbErr(ErrBucketNameRequired, str, nil)
	}
	...
	if b.tx.hasKey(bidxKey) {
		...
		return nil, makeDbErr(ErrBucketExists, str, nil)
	}
...
func (b *bucket) DeleteBucket(key []byte) error {
	...
	if childID == nil {
		...
		return makeDbErr(ErrBucketNotFound, str, nil)
	}
...
func (b *bucket) Put(key, value []byte) error {
	...
	if len(key) == 0 {
		...
		return makeDbErr(ErrKeyRequired, str, nil)
	}
...
func (tx *Transaction) checkClosed() error {
	...
	if tx.closed {
		return makeDbErr(ErrTxClosed, errTxClosedStr, nil)
	}
...
func (tx *Transaction) Metadata() *bucket {
...
func (tx *Transaction) StoreBlock(block *util.Block) error {
	...
	if tx.hasBlock(blockHash) {
		...
		return makeDbErr(ErrBlockExists, str, nil)
	}
	...
	if err != nil {
		str := fmt.Sprintf("failed to get serialized bytes for block %s",
			blockHash)
		return makeDbErr(ErrDriverSpecific, str, err)
	}
...
func (tx *Transaction) fetchBlockRow(hash *daghash.Hash) ([]byte, error) {
	...
	if blockRow == nil {
		...
		return nil, makeDbErr(ErrBlockNotFound, str, nil)
	}
...
func (tx *Transaction) fetchBlockHeaderSize(hash *daghash.Hash) (byte, error) {
	r, err := tx.FetchBlockRegion(&BlockRegion{
		Hash:   hash,
...
func (tx *Transaction) FetchBlockHeader(hash *daghash.Hash) ([]byte, error) {
	...
	return tx.FetchBlockRegion(&BlockRegion{
		Hash:   hash,
		Offset: 0,
...
func (tx *Transaction) fetchBlockHeadersSizes(hashes []*daghash.Hash) ([]byte, error) {
	regions := make([]BlockRegion, len(hashes))
	for i := range hashes {
...
func (tx *Transaction) fetchPendingRegion(region *BlockRegion) ([]byte, error) {
	...
	if endOffset < region.Offset || endOffset > blockLen {
		...
		return nil, makeDbErr(ErrBlockRegionInvalid, str, nil)
	}
...
//
// This function is part of the database.Tx interface implementation.
func (tx *Transaction) FetchBlockRegion(region *BlockRegion) ([]byte, error) {
	// Ensure transaction state is valid.
	if err := tx.checkClosed(); err != nil {
		return nil, err
	}
...
//
// This function is part of the database.Tx interface implementation.
func (tx *Transaction) FetchBlockRegions(regions []BlockRegion) ([][]byte, error) {
	// Ensure transaction state is valid.
	if err := tx.checkClosed(); err != nil {
		return nil, err
	}
...
func (db *DB) begin(writable bool) (*Transaction, error) {
	...
	if db.closed {
		...
		return nil, makeDbErr(ErrDbNotOpen, errDbNotOpenStr,
			nil)
	}
...
func (db *DB) Close() error {
	...
	if db.closed {
		return makeDbErr(ErrDbNotOpen, errDbNotOpenStr, nil)
	}
	db.closed = true
...
func OpenDB(dbPath string, network wire.KaspaNet, create bool) (*DB, error) {
	...
	if !create && !dbExists {
		...
		return nil, makeDbErr(ErrDbDoesNotExist, str, nil)
	}
