...
// values.
func testGetValues(tc *testContext, bucket *bucket, values []keyPair) bool {
	for _, item := range values {
...
func testPutValues(tc *testContext, bucket *bucket, values []keyPair) bool {
	for _, item := range values {
...
// provided bucket.
func testDeleteValues(tc *testContext, bucket *bucket, values []keyPair) bool {
	for _, item := range values {
...
func testCursorInterface(tc *testContext, bucket *bucket) bool {
...
func testNestedBucket(tc *testContext, testBucket *bucket) bool {
...
func testBucketInterface(tc *testContext, bucket *bucket) bool {
...
// testRollbackOnPanic rolls the passed transaction back if the code in the calling
...
func testRollbackOnPanic(t *testing.T, dbTx *Transaction) {
...
func testMetadataManualTxInterface(tc *testContext) bool {
	...
	populateValues := func(writable, rollback bool, putValues []keyPair) bool {
		...
		defer testRollbackOnPanic(tc.t, tx)
...
func testManagedTxPanics(tc *testContext) bool {
	...
	paniced := testPanic(func() {
		tc.db.View(func(dbTx *Transaction) error {
			dbTx.Commit()
			return nil
		})
	})
	...
	paniced = testPanic(func() {
		tc.db.Update(func(dbTx *Transaction) error {
			dbTx.Commit()
			return nil
		})
	})
...
func testMetadataTxInterface(tc *testContext) bool {
	...
	bucket1Name := []byte("bucket1")
	err := tc.db.Update(func(dbTx *Transaction) error {
	...
	err = tc.db.View(func(dbTx *Transaction) error {
...
func testFetchBlockIOMissing(tc *testContext, dbTx *Transaction) bool {
	wantErrCode := ErrBlockNotFound
	...
	allBlockRegions := make([]BlockRegion, len(tc.blocks))
...
func testFetchBlockIO(tc *testContext, dbTx *Transaction) bool {
	// ---------------------
	// Non-bulk Block IO API
	// ---------------------
	...
	for i, block := range tc.blocks {
		...
		testName = fmt.Sprintf("FetchBlockRegion(%s) invalid region",
			blockHash)
		wantErrCode = ErrBlockRegionInvalid
	...
	badBlockRegions := make([]BlockRegion, len(allBlockRegions)+1)
...
func testClosedTxInterface(tc *testContext, dbTx *Transaction) bool {
	wantErrCode := ErrTxClosed
...
func testInterface(t *testing.T, db *DB) {
