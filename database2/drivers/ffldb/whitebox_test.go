...
func checkDbError(t *testing.T, testName string, gotErr error, wantErrCode ErrorCode) bool {
	dbErr, ok := gotErr.(Error)
	if !ok {
...
func TestConvertErr(t *testing.T) {
	...
	tests := []struct {
		...
		{&ldberrors.ErrCorrupted{}, ErrCorruption},
		{leveldb.ErrClosed, ErrDbNotOpen},
		{leveldb.ErrSnapshotReleased, ErrTxClosed},
		{leveldb.ErrIterReleased, ErrTxClosed},
	}
...
func TestCornerCases(t *testing.T) {
	...
	if !checkDbError(t, testName, err, ErrDriverSpecific) {
		return
	}
	...
	wantErrCode = ErrDbNotOpen
	err = idb.View(func(dbTx *Transaction) error {
		return nil
	})
	if !checkDbError(t, testName, err, wantErrCode) {
		return
	...
	err = idb.Update(func(dbTx *Transaction) error {
		return nil
	})
	if !checkDbError(t, testName, err, wantErrCode) {
		return
...
func testWriteFailures(tc *testContext) bool {
	...
	err := tc.db.cache.flush()
	if !checkDbError(tc.t, testName, err, ErrDriverSpecific) {
		return false
	}
	store.writeCursor.Lock()
...
func TestFailureScenarios(t *testing.T) {
	...
	store.openFileFunc = func(fileNum uint32) (*lockableFile, error) {
		...
		if fileNum == ^uint32(0) {
			return nil, makeDbErr(ErrDriverSpecific,
				"test", nil)
		}
		if file, ok := tc.files[fileNum]; ok {
			// "Reopen" the file.
	...
	store.deleteFileFunc = func(fileNum uint32) error {
		...
		return makeDbErr(ErrDriverSpecific, str, nil)
	}
