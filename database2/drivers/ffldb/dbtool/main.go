...
import (
	"github.com/kaspanet/kaspad/database2/drivers/ffldb"
...
func loadBlockDB() (*ffldb.DB, error) {
	// The database name is based on the database type.
	dbName := blockDbNamePrefix + "_ffldb"
	...
	if err != nil {
		...
		// exist.
		var dbErr ffldb.Error
		if ok := errors.As(err, &dbErr); !ok || dbErr.ErrorCode !=
			ffldb.ErrDbDoesNotExist {
