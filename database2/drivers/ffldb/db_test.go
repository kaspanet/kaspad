...
func TestCursorDeleteErrors(t *testing.T) {
	...
	err = pdb.Update(func(dbTx *Transaction) error {
		...
		err := cursor.Delete()
		if !IsErrorCode(err, ErrIncompatibleValue) {
	...
	err = pdb.View(func(dbTx *Transaction) error {
		cursor := dbTx.Metadata().Cursor()
		if !cursor.First() {
		...
		err := cursor.Delete()
		if !IsErrorCode(err, ErrTxNotWritable) {
	...
	err = cursor.Delete()
	if !IsErrorCode(err, ErrTxClosed) {
...
func TestCreateBucketErrors(t *testing.T) {
	...
	tests := []struct {
		...
		{"empty key", []byte{}, true, false, ErrBucketNameRequired},
		{"transaction is closed", testKey, true, true, ErrTxClosed},
		{"transaction is not writable", testKey, false, false, ErrTxNotWritable},
		{"key already exists", blockIdxBucketName, true, false, ErrBucketExists},
	}
	...
	for _, test := range tests {
		func() {
			...
			if !IsErrorCode(err, test.expectedErr) {
...
func TestPutErrors(t *testing.T) {
	...
	tests := []struct {
		...
		{"empty key", []byte{}, true, false, ErrKeyRequired},
		{"transaction is closed", testKey, true, true, ErrTxClosed},
		{"transaction is not writable", testKey, false, false, ErrTxNotWritable},
	}
...
func TestStoreBlockErrors(t *testing.T) {
	...
	tests := []struct {
		...
		{"transaction is closed", true, true, ErrTxClosed},
		{"transaction is not writable", false, false, ErrTxNotWritable},
	}
...
func TestDeleteDoubleNestedBucket(t *testing.T) {
	...
	err := pdb.Update(func(dbTx *Transaction) error {
		...
		rawSecondKey = c.rawKey()
		if dbTx.fetchKey(rawSecondKey) == nil {
